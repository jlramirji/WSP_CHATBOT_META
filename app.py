...
from Funciones_py import leer_archivos,archivos_adjuntos,procesar_audio,crear_log
...
def receive_message():
    try:
        # Log del inicio del procesamiento del webhook
        #crear_log.escribir(LOG_FILE, crear_log.crear_mensaje_log("WEBHOOK", "Iniciando procesamiento de webhook"))
        ...
        if time_difference > 300:  # 5 minutos = 300 segundos
            ...
            #(log.ruta_log, f"Fecha y Hora: {log.fecha_actual} | Mensaje rechazado por ser muy antiguo:{time_difference} | Estado: {log.log_status['log5'][0]}") 
            return "EVENT_RECEIVED"
        ...
        # Log del mensaje recibido en archivo específico
        log_msg = crear_log.log_chat_mensaje(number, message_type, f"ID: {message_id}", "entrada")
        crear_log.escribir_log_sesion(number, log_msg, "CHAT")
        # También escribir en el log general
        crear_log.escribir(LOG_FILE, log_msg)
        
        # Log de información adicional del mensaje
        detalles = f"Timestamp: {datetime.fromtimestamp(message_timestamp).strftime('%Y-%m-%d %H:%M:%S')}, Diferencia: {time_difference}s, Total procesados: {len(getattr(app, 'processed_messages', set()))}"
        # Log de mensaje válido procesado en archivo específico
        info_msg = crear_log.crear_mensaje_log("INFO", "Mensaje válido procesado", number, detalles)
        crear_log.escribir_log_sesion(number, info_msg, "CHAT")
        # También escribir en el log general
        crear_log.escribir(LOG_FILE, info_msg)
        ...
        if message_type == "text":
            ...
            # Log del mensaje de texto recibido
            # Log del mensaje de texto en archivo específico
            log_msg = crear_log.log_chat_mensaje(number, "texto", text[:100] + "..." if len(text) > 100 else text, "entrada")
            crear_log.escribir_log_sesion(number, log_msg, "CHAT")
            # También escribir en el log general
            crear_log.escribir(LOG_FILE, log_msg)
            ...
            # Log del mensaje interactivo en archivo específico
            log_msg = crear_log.log_chat_mensaje(number, "interactivo", text[:100] + "..." if len(text) > 100 else text, "entrada")
            crear_log.escribir_log_sesion(number, log_msg, "CHAT")
            # También escribir en el log general
            crear_log.escribir(LOG_FILE, log_msg)
            ...
            if text and text.strip():
                ...
                # Log de mensaje interactivo vacío
                crear_log.escribir(LOG_FILE, crear_log.crear_mensaje_log("WARNING", "Mensaje interactivo sin texto", number))
                print("⚠️ Mensaje interactivo sin texto, ignorando")
            
        elif message_type in ["audio", "image", "video", "document", "voice"]:
            # Log del archivo adjunto recibido
            log_msg = crear_log.log_chat_mensaje(number, message_type, f"Archivo adjunto recibido", "entrada")
            crear_log.escribir(LOG_FILE, log_msg)
            ...
            if number in user_sessions and user_sessions[number].get('esperando') in ['nota_de_voz', 'esperando_fotosvideos']:
                # Log del procesamiento de archivo
                crear_log.escribir(LOG_FILE, crear_log.crear_mensaje_log("INFO", f"Procesando archivo {message_type}", number))
                ...
                if filepath:
                    if message_type == "audio":
                        ...
                        if user_sessions[number].get('esperando') == 'esperando_fotosvideos':
                            ...
                            # Log del archivo procesado exitosamente
                            crear_log.escribir(LOG_FILE, crear_log.log_exito(f"Archivo {message_type} #{user_sessions[number]['archivos_procesados']} procesado", f"usuario: {number}"))
                    ...
                    # Log del error al procesar archivo
                    crear_log.escribir(LOG_FILE, crear_log.log_error("Error al procesar archivo adjunto", f"tipo: {message_type}, usuario: {number}"))
                ...
                # Log de archivo recibido en estado incorrecto
                crear_log.escribir(LOG_FILE, crear_log.crear_mensaje_log("WARNING", f"Archivo {message_type} recibido en estado incorrecto", number))
        ...
        # Log del error
        error_msg = crear_log.log_error("Error al procesar webhook", "receive_message", e)
        crear_log.escribir(LOG_FILE, error_msg)
...
def registrar_datos_en_bd(number):
    ...
    """
    try:
        # Log del inicio del registro en BD
        crear_log.escribir(LOG_FILE, crear_log.crear_mensaje_log("INFO", "Iniciando registro en base de datos", number))
        
        if number not in user_sessions:
            # Log de error: sesión no encontrada
            crear_log.escribir(LOG_FILE, crear_log.log_error("Sesión no encontrada para registro en BD", f"usuario: {number}"))
        ...
        if innovedad and innovedad.get('idnovedad') is not None:
            # Log de registro exitoso en BD
            crear_log.escribir(LOG_FILE, crear_log.log_exito(f"Datos registrados exitosamente en BD con ID: {incidente_id}", f"usuario: {number}"))
            ...
            if descripcion_archivos:
                if organizar_archivos_adjuntos(number, innovedad['idnovedad']):
                    # Log de archivos organizados
                    crear_log.escribir(LOG_FILE, crear_log.log_exito(f"Archivos adjuntos organizados en carpeta {incidente_id}", f"usuario: {number}"))
                    print(f"📁 Archivos adjuntos organizados en carpeta {incidente_id}")
                else:
                    # Log de error al organizar archivos
                    crear_log.escribir(LOG_FILE, crear_log.log_error("No se pudieron organizar archivos adjuntos", f"incidente: {incidente_id}, usuario: {number}"))
                    print(f"⚠️ No se pudieron organizar archivos adjuntos para incidente {incidente_id}")
            
            # Log de información registrada
            crear_log.escribir(LOG_FILE, crear_log.crear_mensaje_log("INFO", f"Texto del reporte: {texto_reporte[:100]}{'...' if len(texto_reporte) > 100 else ''}", number))
            print(f"📝 Texto del reporte para {number}: {texto_reporte[:100]}{'...' if len(texto_reporte) > 100 else ''}")
            if adjuntos:
                crear_log.escribir(LOG_FILE, crear_log.crear_mensaje_log("INFO", f"Archivos adjuntos: {len(descripcion_archivos)} archivos", number))
                print(f"📎 Archivos adjuntos registrados para {number}:")
                for desc in descripcion_archivos:
                    print(f"   - {desc}")
            else:
                crear_log.escribir(LOG_FILE, crear_log.crear_mensaje_log("INFO", "No hay archivos adjuntos", number))
                print(f"📎 No hay archivos adjuntos para {number}")
            
            return True
        else:
            # Log de error al obtener ID del incidente
            crear_log.escribir(LOG_FILE, crear_log.log_error("Error al obtener ID del incidente", f"usuario: {number}"))
...
def finalizar_sesion_exitosa(number, mensaje_final="✅ Gracias por tu reporte, te esperamos pronto!! 😊"):
    ...
    """
    try:
        # Log del inicio de finalización
        crear_log.escribir(LOG_FILE, crear_log.crear_mensaje_log("SESSION", f"Iniciando finalización exitosa", number))
        ...
        if registrar_datos_en_bd(number):
            ...
            # Log de éxito
            success_msg = crear_log.log_exito("Sesión finalizada exitosamente", f"usuario: {number}")
            crear_log.escribir(LOG_FILE, success_msg)
            ...
            # Log de error
            error_msg = crear_log.log_error("Error al registrar datos en BD", f"usuario: {number}")
            crear_log.escribir(LOG_FILE, error_msg)
            print(f"❌ No se pudo finalizar sesión para {number} - Error en registro BD")
        
    except Exception as e:
        # Log de excepción
        error_msg = crear_log.log_error("Error al finalizar sesión exitosa", f"usuario: {number}", e)
        crear_log.escribir(LOG_FILE, error_msg)
...
def limpiar_datos_sesion(number):
    ...
    try:
        if number in user_sessions:
            ...
            crear_log.escribir(LOG_FILE, crear_log.crear_mensaje_log("INFO", "Iniciando limpieza de datos de sesión", number))
            ...
            crear_log.escribir(LOG_FILE, crear_log.log_exito("Datos de sesión limpiados exitosamente", f"usuario: {number}"))
        ...
        # Log del error en limpieza
        crear_log.escribir(LOG_FILE, crear_log.log_error("Error al limpiar datos de sesión", f"usuario: {number}", e))
...
def verificar_timeout_sesiones():
    ...
    try:
        ...
        for number in sesiones_a_limpiar:
            # Log de sesión por timeout
            crear_log.escribir(LOG_FILE, crear_log.crear_mensaje_log("WARNING", f"Sesión cerrada por timeout ({timeout_segundos}s)", number))
            finalizar_sesion_timeout(number)
            
        if sesiones_a_limpiar:
            # Log de limpieza masiva
            crear_log.escribir(LOG_FILE, crear_log.crear_mensaje_log("INFO", f"Limpieza automática: {len(sesiones_a_limpiar)} sesiones inactivas eliminadas"))
            print(f"🧹 Limpiadas {len(sesiones_a_limpiar)} sesiones inactivas")
            
    except Exception as e:
        # Log del error en verificación de timeout
        crear_log.escribir(LOG_FILE, crear_log.log_error("Error al verificar timeout de sesiones", "verificar_timeout_sesiones", e))
...
def ProcessMesages(text,number):
    ...
    if not text or text.strip() == "":
        # Log de mensaje vacío
        crear_log.escribir(LOG_FILE, crear_log.crear_mensaje_log("WARNING", "Mensaje vacío recibido", number))
        return  # No hacer nada si el mensaje está vacío
    
    # Log del inicio del procesamiento
    crear_log.escribir(LOG_FILE, crear_log.crear_mensaje_log("INFO", f"Iniciando procesamiento de mensaje: {text[:50]}...", number))
    ...
    if "hola" in text or "hello" in text or "hi" in text or "buenas" in text or "buenos día" in text or "buenas tardes" in text or "buenas noches" in text or "inicio" in text:
        ...
        if number not in user_sessions:
            ...
            # Log de nueva sesión en archivo específico
            session_msg = crear_log.log_sesion_usuario(number, "creada", "nueva sesión iniciada")
            crear_log.escribir_log_sesion(number, session_msg, "CHAT")
            # También escribir en el log general
            crear_log.escribir(LOG_FILE, session_msg)
            print(f"🆕 Nueva sesión creada para {number}")
        else:
            # Si ya existe una sesión, reiniciarla
            reiniciar_sesion(number)
            # Log de sesión reiniciada en archivo específico
            session_msg = crear_log.log_sesion_usuario(number, "reiniciada", "sesión existente reiniciada")
            crear_log.escribir_log_sesion(number, session_msg, "CHAT")
            # También escribir en el log general
            crear_log.escribir(LOG_FILE, session_msg)
...
if(__name__ == '__main__'):   
    ...
    crear_log.escribir(LOG_FILE, crear_log.crear_mensaje_log("SYSTEM", "🚀 Aplicación WhatsApp iniciada", None, f"Puerto: 5000, Debug: True"))
